...
async function UpsertOrder(purchase) {
    ...
    if(!purchase.orderNumber){
        ...
        if(purchaseOrder.statusCode > orderStatus.Updated.code){
            throw new Error("Order cannot be updated");
        }
    }

    purchaseOrder.id = purchase.id;
    purchaseOrder.items = [];
    ...
    purchaseOrder.amount = purchase.amount || '0';

    if(purchase.checkerDependency){
        purchaseOrder.buyerChecker = purchase.buyerChecker || null;
        purchaseOrder.sellerChecker = purchase.sellerChecker || null;
    }
    ...
    if(!created){
        ...
        purchaseOrder.statusCode = orderStatus.Updated.code;
        purchaseOrder.statusMessage = orderStatus.Updated.text;
...
async function SubmitOrder(purchase) {
    ...
    if(!purchaseOrder) throw new Error("Purchase Order : "+ purchase.orderNumber +" not found !!!");
    if(purchaseOrder.statusCode >  orderStatus.Submit.code){
    ...
    purchaseOrder.updated = new Date().toISOString();
    purchaseOrder.statusCode = orderStatus.Submit.code;
    purchaseOrder.statusMessage = orderStatus.Submit.text;
...
async function CancelOrder(purchase) {
        ...
        purchaseOrder.message = purchase.message;
...
async function POApproval(purchase) {
    ...
    purchaseOrder.approved = new Date().toISOString();
...
/**
 * Record a request to approve PO by seller checker
 * param {io.aeries.art.order.POApprovalChecker} purchase
 * transaction
 */
/*
async function POApprovalChecker(purchase) {
    if(!purchase.checkerDependency){
    ...
    var sellerCheckerParticipantRegistry = await getParticipantRegistry('io.aeries.art.participant.SellerChecker');
    var sellerChecker = await sellerCheckerParticipantRegistry.get(currentParticipant.getIdentifier());
    //if(!sellerChecker.verified){
    //	throw new Error("Seller Checker not verified");
    //}
    ...
    var purchaseOrder = await assetRegistry.get(purchase.orderNumber);
    if(!purchaseOrder) throw new Error("Purchase Order : "+ purchase.orderNumber +" not found !!!");
    if(purchaseOrder.statusCode != orderStatus.Approve.code){
        throw new Error("Order cannot be approved by checker");
    }

    purchaseOrder.approvedChecker = new Date().toISOString();
    purchaseOrder.statusCode = orderStatus.ApproveChecker.code;
    purchaseOrder.statusMessage = orderStatus.ApproveChecker.text;
